    ...
    for idx_set = 1:num_set
        for scale = scale_all
            ...
            for idx_ext = 1:length(ext)
                filepaths = cat(1, filepaths, dir(fullfile('./dataset', dataset{idx_set}, [dataset{idx_set}, '_HR'], ext{idx_ext})));
            end
            PSNR_all = zeros(1, length(filepaths));
            SSIM_all = zeros(1, length(filepaths));
            for idx_im = 1:length(filepaths)
                name = filepaths(idx_im).name;
                name_HR = strrep(name, '.bmp', ['_x', num2str(scale), '_HR', '.png']);
                name_SR = strrep(name, '.bmp', ['_x', num2str(scale), '_SR', '.png']);
                im_HR = imread(fullfile('./experiment/test/', ['results-', dataset{idx_set}], name_HR));
                im_SR = imread(fullfile('./experiment/test/', ['results-', dataset{idx_set}], name_SR));
