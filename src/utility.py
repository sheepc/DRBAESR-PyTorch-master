...
class checkpoint():
    ...
    def save(self, trainer, epoch, is_best=False):
        ...
        trainer.loss[stage].save(self.dir)
    ...
    def save_test_results(self, dataset, filename, save_list, scale):
    ...
    def save_train_results(self, stage, dataset_name, filename_list, img_list, scale):
        prefix_list = ["lr", "hr", "lr_recons", "hr_recons", "lr_z", "hr_z", "sr"]
        # prefix_list = ["lr", "hr", "lr_recons", "hr_recons", "sr"]
        if stage == 0:
            file_path_root = self.args.first_stage_result_dir
        if stage == 1:
            file_path_root = self.args.second_stage_result_dir
        if stage == 2:
            file_path_root = self.args.third_stage_result_dir

        dir_path = os.path.join(file_path_root, dataset_name[0], 'x{}'.format(scale[0]))
        ...
        for file_index, filename in enumerate(filename_list):  # filename for a batch
            imagesOfFile = [img_list[0][file_index], img_list[1][file_index], img_list[2][file_index],
                            img_list[3][file_index], img_list[4][file_index], img_list[5][file_index], img_list[6][file_index]]
